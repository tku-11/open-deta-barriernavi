...
- **パスワードリセット**: メール送信・トークン機構が未実装のため、現在は利用不可
...
- **聴覚障害向け**: 4項目（すべてフラグ型）
- **視覚障害向け**: 10項目（フラグ型6項目、割合型1項目、数値型3項目）
...
- `PATCH /api/auth/profile` - プロフィール情報の部分更新（正規メソッド）
- `PUT /api/auth/profile` - `PATCH` と同じ部分更新として後方互換で受け付け

### 駅情報関連

- `GET /api/body/stations` - 身体障害向け駅一覧取得（スコア付き）
  - クエリ: `keyword`, `prefecture`, `line_name`, `limit`（1〜100）, `offset`, `filters`（評価項目キーのJSON配列）, `sort`（`score-asc` / `score-desc`）
  - `filters` は評価基準を満たす駅だけを残し、複数指定時はAND条件です。
...
- CORSは既定で無効です。別オリジンを許可する場合だけ、`CORS_ALLOWED_ORIGINS`に管理下のHTTPSオリジンを明示してください
...
- `docs/refactoring/05-p1-contract.md` - スコア、フィルタ、プロフィール更新の正規契約
- `プログラム概要.txt` - 初心者向けプログラム説明
