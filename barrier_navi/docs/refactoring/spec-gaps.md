...
視覚カテゴリは 10 項目として正式化し、README とスコア説明書を実装に同期した。
...
重み付けなしの単純カウントを正式仕様とし、`weights` は公開API仕様から除外した。将来導入する場合は、新しいスコア仕様と回帰テストを先に追加する。
...
`filters` を正式なクエリ名とし、README の一覧API説明を更新した。
...
数値型フィルタを `required` 以上へ変更し、フラグ・割合を含めてスコア達成条件と一致させた。
...
`PATCH /api/auth/profile` を正規メソッドとし、フィールド省略時は保持、空配列は明示クリア、不正型は 400 とした。`PUT` は同じ部分更新として後方互換で受け付ける。
